...
public class GitUtilities : Common
{
    ...
    public static Task CloneRepositoryTask(string groupPath, string repoPath, string gitSsh)
    {
        try
        {
            string groupFullPath = AppSettingsUtilities.Configuration["DownloadPath"] + groupPath;
            string projectPath = AppSettingsUtilities.Configuration["DownloadPath"] + repoPath + ".git";
            ...
            {
                FileName = "git",
                Arguments = $"clone --mirror {gitSsh}",
                WorkingDirectory = groupFullPath
            };
