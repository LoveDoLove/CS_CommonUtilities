using CommonUtilities.Config; // Assuming Config is a class for managing configuration settings.
using CommonUtilities.Models; // Assuming ReadLineModel is defined here.

namespace CommonUtilities.Helpers;

/// <summary>
/// Provides static helper methods for console input and output operations,
...
public static class ConsoleHelper
{
    ...
    /// <param name="config">Optional configuration object to retrieve 'text_color' and 'background_color' settings.
    /// Colors should be valid <see cref="ConsoleColor"/> enum names.</param>
    public static void WriteInfo(string message, CommonUtilities.Config.Config? config = null)
    {
        ...
        if (config is not null)
        {
            ...
            var textColorObj = config.Get("text_color", string.Empty); // Provide a default to avoid null issues with ToString
            if (textColorObj != null && !string.IsNullOrEmpty(textColorObj.ToString()) && Enum.TryParse(textColorObj.ToString(), true, out ConsoleColor parsedTextColor))
            {
            ...
            if (bgColorObj != null && !string.IsNullOrEmpty(bgColorObj.ToString()) && Enum.TryParse(bgColorObj.ToString(), true, out ConsoleColor parsedBgColor))
            {
    ...
    /// <param name="config">Optional configuration object to retrieve the 'error_color' setting.
    /// The color should be a valid <see cref="ConsoleColor"/> enum name.</param>
    ...
    /// <param name="config">Optional configuration object to retrieve the 'success_color' setting.
    /// The color should be a valid <see cref="ConsoleColor"/> enum name.</param>
    ...
    /// </summary>
    /// <param name="readLineModel">A <see cref="ReadLineModel"/> object containing the prompt question,
    /// a flag indicating if empty input is allowed, and a default value to return on cancellation.</param>
