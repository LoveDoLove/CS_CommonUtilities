...
/// <summary>
/// Provides helper methods for working with enumerations (enums).
...
public static class EnumHelper
{
    ...
    /// Uses the <see cref="DisplayAttribute.Name"/> for the text of each item if available, otherwise uses the enum member's string representation.
    /// </summary>
    /// <typeparam name="T">The type of the enumeration.</typeparam>
    ...
    public static SelectList ToSelectList<T>(string? selectedValue = null) where T : Enum
    {
        var enumList = Enum.GetValues(typeof(T))
            .Cast<T>()
            .Select(e => new { Value = e.ToString(), Text = GetDisplayName(e) }) // Project to an anonymous type with Value and Text
            .ToList();
    ...
    /// <param name="allowedValues">An array of enum values that are allowed in the <see cref="SelectList"/>. If null or empty, the resulting list will be empty.</param>
    ...
    /// <typeparam name="T">The type of the enumeration.</typeparam>
    /// <param name="selectedValue">Optional. The value that should be pre-selected.</param>
    /// <param name="valueToExclude">The enum value to exclude from the <see cref="SelectList"/>. If null, no value is excluded.</param>
        ...
        where T : Enum
    {
        ...
        {
            enumListQuery = enumListQuery.Where(e => !e.Equals(valueToExclude));
        }
    ...
    /// <typeparam name="T">The type of the enumeration.</typeparam>
    /// <param name="selectedValue">Optional. The value that should be pre-selected.</param>
    /// <param name="valuesToExclude">An array of enum values to exclude from the <see cref="SelectList"/>. If null or empty, no values are excluded.</param>
    ...
    public static bool IsAllowedValue<T>(T currentValue, T[]? allowedValues = default) where T : struct, Enum // 'struct' constraint for enums
    {
