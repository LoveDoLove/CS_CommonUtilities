...
using System.Linq;
using CommonUtilities.Utilities; // Assuming LoggerUtilities is here

namespace CommonUtilities.Helpers
{
    /// <summary>
    /// Provides helper methods for finding executable files.
    /// </summary>
    public static class ExecutablePathHelper
    {
        ...
        /// <summary>
        /// Attempts to find an executable by checking an explicitly configured path, then the system PATH, and then additional provided search directories.
        /// </summary>
        /// <param name="executableName">The name of the executable (e.g., "node", "php").</param>
        /// <param name="configuredPath">An explicitly configured path for the executable. If provided and valid, it will be used directly.</param>
        /// <param name="additionalSearchDirectories">A collection of additional directories to search if not found in configuredPath or system PATH.</param>
        /// <returns>The full path to the executable if found; otherwise, null.</returns>
        public static string? FindExecutable(string executableName, string? configuredPath = null, IEnumerable<string>? additionalSearchDirectories = null)
        {
            if (string.IsNullOrWhiteSpace(executableName))
            {
            ...
            if (!string.IsNullOrWhiteSpace(configuredPath))
            {
                try
                {
                    ...
                    {
                        LoggerUtilities.Info($"Configured path '{configuredPath}' for '{executableName}' does not exist or is not a file.");
                    }
                }
                catch (Exception ex)
                {
                ...
                foreach (var dir in additionalSearchDirectories)
                {
                    ...
                    foreach (var ext in extensionsToSearch)
                    {
                        ...
                        try
                        {
                            if (File.Exists(potentialPath))
                            {
                                LoggerUtilities.Info($"Executable '{executableName}' found in additional directory '{dir}': {potentialPath}");
                                return Path.GetFullPath(potentialPath);
                            }
                        }
                        catch (Exception ex)
                        {
                             LoggerUtilities.Error(ex, $"Error checking file existence for {potentialPath} in additional directory {dir}");
                        }
                    }
                }
            ...
            LoggerUtilities.Info($"Executable '{executableName}' not found after checking configured path, system PATH, and additional directories.");
            return null;
        }
    }
}