...
namespace CommonUtilities.Helpers
{
    ...
    public static class ProcessExecutionHelper
    {
        ...
        /// <param name="loadUserProfile">For Windows, whether to load the user's profile. Defaults to true if OS is Windows, false otherwise.</param>
        /// <returns>A tuple containing (string output, string error, int exitCode).</returns>
        public static (string output, string error, int exitCode) RunProcessAndCaptureOutput(string executablePath, string arguments, string? workingDirectory = null, bool? loadUserProfile = null)
        {
            ...
            {
                string message = $"Win32Exception (NativeErrorCode: {ex.NativeErrorCode}) while trying to run '{executablePath} {arguments}'. Message: {ex.Message}";
                if (ex.NativeErrorCode == 2) // ERROR_FILE_NOT_FOUND
                {
                    message = $"Executable not found: '{executablePath}'. Ensure it is correctly installed and in PATH or configured path.";
                }
        ...
        public static int RunProcessWithStreaming(string executablePath, string arguments, string? workingDirectory = null, bool? loadUserProfile = null, Action<string>? outputDataReceived = null, Action<string>? errorDataReceived = null)
        {
            bool actualLoadUserProfile = loadUserProfile ?? (Environment.OSVersion.Platform == PlatformID.Win32NT);
            int exitCode = -1;
            ...
            try
            {
                using (Process process = new Process())
                {
                    ...
                    {
                        process.OutputDataReceived += (sender, e) => { if (e.Data != null) outputDataReceived(e.Data); };
                    }
                    else
                    {
                    ...
                    {
                        process.ErrorDataReceived += (sender, e) => { if (e.Data != null) errorDataReceived(e.Data); };
                    }
                    else
                    {
