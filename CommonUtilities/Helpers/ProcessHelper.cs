...
public static class ProcessHelper
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (await Task.WhenAny(processTask, timeoutTask) == timeoutTask)
            {
                if (!process.HasExited) // Check if the process is still running
                {
                    ...
                    catch (Win32Exception) { /* Process could not be terminated or is exiting */ }
                    // It's important to catch specific exceptions here to avoid masking other issues.
                    throw new TimeoutException($"Process timed out after {timeoutMilliseconds / 1000} seconds: {fileName} {arguments}");
                }
            }
        ...
        catch (Win32Exception ex) // Handle errors like "file not found" or permission issues
        {
            ...
            throw new InvalidOperationException($"Error starting process '{fileName}'. Ensure the command is correct and accessible. Details: {ex.Message}", ex);
        }
        catch (Exception ex) // Catch-all for other unexpected errors during process execution
        {
            ...
            throw new Exception($"An unexpected error occurred while running process '{fileName} {arguments}'. Details: {ex.Message}", ex);
        }
    }
    ...
    private static string[] GetSearchPathsForExecutable(string executableName)
    {
        ...
        if (Environment.OSVersion.Platform == PlatformID.Win32NT) // Windows-specific paths
        {
            ...
            string appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData); // For user-specific installations
            ...
            switch (lowerExecutableName)
            {
                ...
                case "node":
                    ...
                    paths.Add(Path.Combine(programFilesX86, "nodejs", $"{executableName}{extension}")); // For 32-bit Node.js on 64-bit Windows
                    paths.Add(Path.Combine(appData, "npm", $"{executableName}.cmd")); // npm often uses .cmd on Windows
                    paths.Add(Path.Combine(appData, "npm", executableName)); // Sometimes it's just 'npm' without extension in PATH
                    break;
                case "php":
                    ...
                    paths.Add(Path.Combine("C:\\", "Program Files", "php", $"{executableName}{extension}")); // Generic PHP install
                    break;
                case "composer":
                    // Composer paths can vary, these are common ones
                    paths.Add(Path.Combine(appData, "Composer", "vendor", "bin", "composer.bat"));
                    paths.Add(Path.Combine(appData, "Composer", "vendor", "bin", "composer")); // For git bash or similar
                    paths.Add(Path.Combine("C:\\", "ProgramData", "ComposerSetup", "bin", "composer.bat")); // System-wide install
        ...
        else // Linux/macOS specific paths
        {
            ...
            switch (lowerExecutableName)
            {
                ...
                case "php":
                    ...
                    paths.Add(Path.Combine("/opt", "homebrew", "bin", executableName)); // For Homebrew on Apple Silicon macOS
                    break;
                case "composer":
        ...
        paths.Add(executableName + (Environment.OSVersion.Platform == PlatformID.Win32NT && !lowerExecutableName.EndsWith(".exe") && !lowerExecutableName.EndsWith(".bat") && !lowerExecutableName.EndsWith(".cmd") ? extension : ""));
    ...
    public static (List<string> missingExtensions, bool isCompatible) CheckPhpExtensions(string phpPath = "php")
    {
        ...
        if (exitCode == 0 && !string.IsNullOrWhiteSpace(output))
        {
            ...
            var loadedExtensions = output.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                ...
                .Where(ext => !string.IsNullOrWhiteSpace(ext) && !ext.StartsWith("[") && !ext.Contains(" ")) // Filter out section headers and empty lines
                .ToList();
            ...
            if (missingExtensions.Any(ext => criticalExtensions.Contains(ext.ToLowerInvariant())))
            {
                isCompatible = false;
            }
        }
    ...
    public static (string version, string path, bool isGlobal) GetComposerInfo()
    {
        ...
        if (composerExecutable == "php" && argumentsToUse.Contains(".phar"))
        {
            ...
            if (pharPathMatch.Success)
            {
                finalPath = $"php {pharPathMatch.Groups[1].Value}";
            }
