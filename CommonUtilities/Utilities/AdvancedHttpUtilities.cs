...
/// </summary>
public static class AdvancedHttpUtilities
{
    /// <summary>
    ...
    ///     Optional User-Agent string for the HTTP requests. If null or empty, a default User-Agent will
    ///     be used.
    /// </param>
    /// <param name="timeoutSeconds">
    ///     Optional timeout in seconds for HTTP requests. Defaults to
    ///     <see cref="DefaultTimeoutSeconds" /> (30 seconds).
    /// </param>
    ...
    /// <summary>
    ///     Executes a <see cref="RestRequest" /> with automatic retry logic for transient failures (e.g., network issues, rate
    ...
    ///     Thrown if the request fails after all retries for reasons other than HTTP errors
    ///     handled by RestSharp (e.g. network connectivity issues).
    /// </exception>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (true) // Loop indefinitely until success, max retries exceeded, or unrecoverable error
            try
            {
                ...
                {
                    Log.Information("Request to {Resource} succeeded with status {StatusCode}", request.Resource,
                        response.StatusCode);
                    return response;
                }
                ...
                if (response.StatusCode == HttpStatusCode.TooManyRequests) // HTTP 429
                {
                    ...
                    {
                        Log.Warning(
                            "Rate limit hit for {Resource}. Waiting {DelayMs}ms before retry {RetryCount}/{MaxRetries}. Status: {StatusCode}",
                        ...
                        delay = TimeSpan.FromMilliseconds(delay.TotalMilliseconds *
                                                          2); // Exponential backoff: double the delay
                ...
                else if (IsTransientError(response.StatusCode)) // Check for other transient errors (e.g., 5xx, timeout)
                {
                    ...
                    {
                        Log.Warning(
                            "Transient error {StatusCode} for {Resource}. Waiting {DelayMs}ms before retry {RetryCount}/{MaxRetries}",
                ...
                Log.Error(
                    "API request to {Resource} failed with status {StatusCode}: {ErrorMessage}. Content: {Content}",
            ...
            catch (OperationCanceledException ex)
            {
                ...
                Log.Warning(ex, "Operation cancelled for {Resource} during attempt {AttemptNumber}", request.Resource,
                    retryCount + 1);
                throw;
            }
            catch (Exception ex) when
                (!(ex is HttpRequestException)) // Catch other exceptions (e.g., network issues not resulting in an HTTP response)
            {
                retryCount++;
                if (retryCount <= maxRetries)
                {
                    Log.Warning(ex,
                        "Network or unexpected error for {Resource}. Waiting {DelayMs}ms before retry {RetryCount}/{MaxRetries}",
                ...
                Log.Error(ex,
                    "API request to {Resource} failed after {RetryCount} retries due to network or unexpected error.",
                    request.Resource, retryCount);
    ...
    ///     or default(<typeparamref name="T" />) if the request fails, is unsuccessful, or the response data is null.
    /// </returns>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Log.Warning(
            "GetJsonAsync for {BaseUrl}{Endpoint} failed or returned null data. Status: {StatusCode}, Error: {ErrorMessage}",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Log.Warning(
            "PostJsonAsync for {BaseUrl}{Endpoint} failed or returned null data. Status: {StatusCode}, Error: {ErrorMessage}",
            baseUrl, endpoint, response.StatusCode, response.ErrorMessage ?? "N/A");
        return default; // Return default(T) if not successful or data is null
    }

    /// <summary>
    ///     Makes an HTTP request (GET, POST, PUT, DELETE) to the specified endpoint and deserializes the JSON response into a
    ///     list of objects.
    ///     Uses <see cref="ExecuteWithRetryAsync{T}" /> for robust execution.
    /// </summary>
    ...
    /// <param name="method">
    ///     The HTTP method to use (e.g., <see cref="System.Net.Http.HttpMethod.Get" />,
    ///     <see cref="System.Net.Http.HttpMethod.Post" />). Defaults to GET.
    /// </param>
    /// <param name="headers">Optional dictionary of custom headers to include in the request.</param>
    ...
    ///     or an empty list if the request fails, is unsuccessful, or the response data is null or empty.
    /// </returns>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var restMethod = (method ?? HttpMethod.Get).Method.ToUpperInvariant() switch
        {
            "POST" => Method.Post,
            "PUT" => Method.Put,
        ...
        if (payload != null && (restMethod == Method.Post || restMethod == Method.Put)) request.AddJsonBody(payload);

        var response = await ExecuteWithRetryAsync<List<T>>(client, request, cancellationToken);
        if (response.IsSuccessful && response.Data != null) return response.Data;

        Log.Warning(
            "GetJsonListAsync for {BaseUrl}{Endpoint} (Method: {Method}) failed or returned null/empty data. Status: {StatusCode}, Error: {ErrorMessage}",
